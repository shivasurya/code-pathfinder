...
`)
	...
	// Extract assignments
	err = ExtractVariableAssignments(filePath, sourceCode, typeEngine, modRegistry, typeEngine.Builtins, nil, nil)
	assert.NoError(t, err)
...
func TestInferTypeFromExpression(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = ExtractVariableAssignments(filePath, sourceCode, typeEngine, modRegistry, builtinRegistry, nil, nil)
			assert.NoError(t, err)
