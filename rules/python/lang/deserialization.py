...
```python
import pickle, yaml
# Pickle RCE from untrusted network data
data = pickle.loads(network_data)
# YAML object instantiation from user input
config = yaml.load(user_yaml_string)
# jsonpickle from untrusted API response
obj = jsonpickle.decode(api_response)
```
