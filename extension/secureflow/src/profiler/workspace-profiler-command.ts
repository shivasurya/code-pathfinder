...
export class WorkspaceProfilerCommand {
  ...
  private async executeCommand(): Promise<void> {
    ...
    const outputChannel = vscode.window.createOutputChannel('SecureFlow Workspace Profiler');
    outputChannel.clear();
    ...
    await vscode.window.withProgress({
      ...
      try {
        outputChannel.appendLine('🔍 Profiling workspace to identify application types...');

        outputChannel.appendLine(`📂 Workspace: ${workspaceFolder.name}`);
        
        // Create profiler instance with settings manager
        const profiler = new ProjectProfiler(workspaceFolder, this.settingsManager);
        ...
        if (applications.length === 0) {
          ...
          vscode.window.showWarningMessage('Could not determine the application type. Manual configuration may be required.');
          return;
        }
        ...
        if (applications.length > 1) {
          outputChannel.appendLine(`📊 Detected ${applications.length} applications in this workspace`);
          
          // Create quickpick for user to select
          const selected = await this.promptUserForApplicationSelection(applications);
          if (!selected) {
            outputChannel.appendLine('❌ Application selection cancelled');
            vscode.window.showInformationMessage('Application selection cancelled.');
            return;
          }
        ...
        for (const app of selectedApplications) {
          ...
          outputChannel.appendLine(`Type: ${app.category}${app.subcategory ? '/' + app.subcategory : ''}`);
          outputChannel.appendLine(`Technology: ${app.technology || 'N/A'}`);
          outputChannel.appendLine(`Languages: ${app.languages.join(', ')}`);
          outputChannel.appendLine(`Frameworks: ${app.frameworks.join(', ')}`);
          outputChannel.appendLine(`Build Tools: ${app.buildTools.join(', ')}`);
        ...
        outputChannel.appendLine(`❌ Error during workspace profiling: ${error}`);
        vscode.window.showErrorMessage('Error during workspace profiling. See output for details.');
      }
    });
  }
   ...
   */
  private async promptUserForApplicationSelection(applications: ApplicationProfile[]): Promise<ApplicationProfile | undefined> {
