...
func AnalyzeIntraProceduralTaint(
	...
	summary := NewTaintSummary(functionFQN)
	...
	for _, stmt := range statements {
		...
		if isSource(stmt, sources) {
			...
			if stmt.Def != "" {
				...
				// Add to TaintedVars
				summary.AddTaintedVar(stmt.Def, &TaintInfo{
		...
		if stmt.Type == StatementTypeAssignment {
			propagateAssignment(stmt, taintState, summary)
		}

		// Handle CALL propagation
		if stmt.Type == StatementTypeCall || stmt.CallTarget != "" {
...
func propagateAssignment(stmt *Statement, taintState *TaintState, summary *TaintSummary) {
	if stmt.Def == "" {
		return
	}
...
func propagateCall(stmt *Statement, taintState *TaintState, summary *TaintSummary) {
	if stmt.Def == "" {
		return
	}
...
func handleSanitizer(stmt *Statement, taintState *TaintState) {
	if stmt.Def != "" {
...
func isSource(stmt *Statement, sources []string) bool {
	if stmt.CallTarget == "" {
		return false
	}
...
func isSink(stmt *Statement, sinks []string) bool {
	if stmt.CallTarget == "" {
		return false
	}
...
func isSanitizer(stmt *Statement, sanitizers []string) bool {
	if stmt.CallTarget == "" {
		return false
	}
